...
function useBookDetail(slug) {
  const localStorageKey = "books"; // Key penyimpanan daftar buku

  // Gunakan useMemo untuk memastikan storedBooks tidak berubah di setiap render
  const storedBooks = useMemo(() => {
    const books = localStorage.getItem(localStorageKey);
    return books ? JSON.parse(books) : [];
  }, []);
  // Cari buku berdasarkan slug dari storedBooks
  const foundBook = useMemo(() => storedBooks.find((book) => book.slug === slug), [storedBooks, slug]);
  
  // Hanya fetch data jika buku tidak ditemukan di localStorage
  const { data, error, isValidating } = useSWR(
    foundBook ? null : `${BASE_URL}/api/catalogue/getDetails?slug=${slug}`,
    fetcher,
    {
  ...
  // Gunakan data dari localStorage jika ada, jika tidak gunakan hasil fetch dari API
  const book = foundBook ?? data?.results ?? [];
  const loadingBook = isValidating;

  // Simpan hasil fetch ke localStorage setelah mendapatkan data dari API
  useEffect(() => {
    if (data?.results) {
      const updatedBooks = [...storedBooks.filter((b) => b.slug !== slug), data.results]; // Update list tanpa duplikasi
      localStorage.setItem(localStorageKey, JSON.stringify(updatedBooks));
    }
  }, [data, slug, storedBooks]);
