...
const SectionKurator = () => {
    ...
    const showDesk = (id) => {
        const index = this.currators.findIndex(object => {
            return object.id === id;
        })
        this.currators[index].show_desk = true
    } 
    ...
    const hideDesk = (id) => {
        ...
        this.currators[index].show_desk = false
    }
                                ...
                                <tbody>
                                    {
                                        currators.map((currator, index) => (
                                            <tr key={index}>
                                                <td>{currator.name}</td>
                                                <td>{currator.description}</td>
                                                {/* {
                                                    !currator.showDesk ? (
                                                        <td>
                                                            {currator.description} &nbsp;
                                                            <button class="text-blue bg-transparent border-0 ps-0" onClick={() => showDesk(currator.id)}>selengkapnya &gt;</button>
                                                        </td>
                                                    ) : (
                                                        <td>
                                                            {currator.description} 
